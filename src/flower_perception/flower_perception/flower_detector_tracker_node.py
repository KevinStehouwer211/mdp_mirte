...
class FlowerDetector(Node):
    def __init__(self):
        super().__init__("flower_detector_tracker")
    ...
    def image_callback(self, msg: CompressedImage):
        ...
        for box in result.boxes:
            ...
            # Asign track ID to class ID to distinguish between different 
            # instances of the same class
            track_id = -1
            if box.id is not None:
                track_id = int(box.id[0])

            # Asign Track ID, class ID and confidence to hypothesis
            hypothesis.hypothesis.class_id = f"{class_id}_track_{track_id}"
